#!/bin/bash
# Linux Optimizer — CentOS Stream / AlmaLinux / Rocky Linux / CloudLinux entry point.
#
# Thin wrapper kept so the historical one-liners
#   wget .../scripts/centos-optimizer.sh && bash centos-optimizer.sh
...
case "centos almalinux rocky cloudlinux rhel " in
    *" $OS_ID "*) : ;;
    *)
        red_msg "This entry point is for CentOS Stream / AlmaLinux / Rocky Linux / CloudLinux, but you are running $(os_label)."
        echo
...
lo_banner "CentOS Stream / AlmaLinux / Rocky Linux / CloudLinux detected: $(os_label)"
export LO_RAW_BASE
