#!/bin/bash
# Linux Optimizer — Debian entry point.
#
# Thin wrapper kept so the historical one-liners
#   wget .../scripts/debian-optimizer.sh && bash debian-optimizer.sh
...
case "debian " in
    *" $OS_ID "*) : ;;
    *)
        red_msg "This entry point is for Debian, but you are running $(os_label)."
        echo
...
lo_banner "Debian detected: $(os_label)"
export LO_RAW_BASE
